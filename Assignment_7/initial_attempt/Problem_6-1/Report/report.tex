...
Build and train a feedforward neural network in PyTorch to predict warehouse picking times from operational features. The dataset contains nonlinear interactions that a linear model cannot capture. This experiment demonstrates when and why neural networks outperform linear models
by examining the Cross-Validation Loss Curves, Predicted-vs.-Actual scatter plots, RMSE comparison, and a depth experiment using PyTorch.
}
\end{adjustwidth}
\vspace{9pt}
\hrule
...
\subsection{Neural Network Training and Cross-Validation}
...
We plotted the Cross-Validation (CV) loss curves for each fold to visualize the learning process. 
We also created scatter plots of the predicted vs. actual picking times for both the linear model and the neural network to visually assess prediction quality. 
Finally, we computed and compared the RMSE for both models on the test set.
Collectively, we will use these metrics to demonstrate the superiority of the neural network over the linear model in capturing the underlying data patterns and making accurate predictions.
...
\begin{tabular}{lccc}
\hline
\textbf{Model} & \textbf{Test RMSE (sec)} & \textbf{Relative} & \textbf{Improvement (\%)} \\
\hline
